.. EditGroups documentation master file, created by
   sphinx-quickstart on Sun May 19 09:49:18 2019.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Welcome to EditGroups's documentation!
======================================

EditGroups is an application that groups Wikidata edits into logical batches.
The purpose of this grouping is to give a brids-eye overview over data imports
and maintenance tasks happening in Wikidata, provide a simple mechanism to refer
to and discuss these batches, and undo them in a few clicks.

Documentation for users can be found on Wikidata itself. This manual is for developers
who want to contribute to EditGroups, interact with it through its API, extend it,
deploy it on other wikis, and so on.

.. toctree::
   :maxdepth: 2
   :caption: Contents:

   architecture
   install
   api
   tagging
   reverting
   other_wiki
   testing
   docs


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`
